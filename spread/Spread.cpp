...
float CSpread::GetSpread(CBaseEntity *pEntity, Vector &vecSrc, Vector &vecDirShooting, float vecSpread, float flDistance, int iPenetration, int iBulletType, int iDamage, float flRangeModifier, entvars_t *pevAttacker, bool bPistol, int shared_rand)
{
    ...
    CBasePlayer *Player = EntityToValidPlayer(pEntity);
    ...
    // --- Ground Check ---
    if (this->m_GroundCheck && this->m_GroundCheck->value > 0.0f)
    {
        if (DebugMode)
        {
            this->ClientPrint(
    ...
    // --- Max Speed Check ---
    if (this->m_MaxSpeed && this->m_MaxSpeed->value > 0.0f)
    {
        if (Player->pev->maxspeed > 0.0f)
        {
            float speedLimit = Player->pev->maxspeed / this->m_MaxSpeed->value;
            ...
            if (DebugMode)
            {
                this->ClientPrint(
                    ...
                    Player->pev->maxspeed,
                    (Player->pev->velocity.Length2D() <= speedLimit) ? "Yes" : "No"
                );
            }

            if (Player->pev->velocity.Length2D() > speedLimit)
            {
                return vecSpread;
            }
        }
    ...
    // --- Max Punch Angle Check ---
    if (this->m_MaxPunchAngle && this->m_MaxPunchAngle->value >= 0.0f)
    {
        if (DebugMode)
        {
            this->ClientPrint(
    ...
    // --- Blocked Weapons Check ---
    if (this->m_Weapons && this->m_Weapons->string && this->m_Weapons->string[0u] != '\0')
    {
        if (Player->m_pActiveItem)
        {
            ...
            {
                size_t strLen = strlen(this->m_Weapons->string);

                if ((size_t)weaponId < strLen)
                {
                    if (DebugMode)
                    {
                        this->ClientPrint(
    ...
    // --- Apply custom spread ---
    if (this->m_Spread && this->m_Spread->value >= 0.0f)
    {
        float newSpread = this->m_Spread->value;

        if (!std::isfinite(newSpread) || newSpread < 0.0f)
        {
            return vecSpread; 
        }
